...
def Let_us_play(stacks):
    ...
    return enough_occurred, game_over_occurred
...
def battle(p_hand, d_hand):  
    global dealer_cards, bet, result, player_results, deposit, deposit_curve, consecutive_losses
    ...
    if result == 'BJ':
        ...
        bet *= 2
        if bet > deposit:
            bet = deposit // 2 #originally bet = deposit
        #if consecutive_losses >= loss_threshold: ##added threshold
            #reset_betting()
...
for i in range(10):
    deposit_curve = []
    enough_occurred, game_over_occurred = Let_us_play(10000)
    all_enough_flags.append(enough_occurred)
    all_game_over_flags.append(game_over_occurred)

print("Enough occurred in simulations:", sum(all_enough_flags))
print("Game Over occurred in simulations:", sum(all_game_over_flags))
"""
